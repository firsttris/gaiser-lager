...
function FlowChoiceCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-3xl border p-7 text-left shadow-[0_12px_28px_rgba(15,23,42,0.05)] transition hover:-translate-y-0.5 sm:p-8 ${
        isPickup
    ...
    >
      <div className="flex min-h-46 items-center gap-5 sm:min-h-52 sm:gap-6">
        ...
        >
          {isPickup ? <ArrowDownToLine className="h-7 w-7" strokeWidth={2.2} /> : <ArrowUpFromLine className="h-7 w-7" strokeWidth={2.2} />}
        </div>
      </div>
    </button>
  )
