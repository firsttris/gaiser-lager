            ...
            <div className="mt-4 space-y-3 md:hidden">
              {filteredRecords.map((record) => (
                <article key={record.id} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <button
                    type="button"
                    onClick={() => downloadDeliveryNote(record, selectedCompany?.name ?? '')}
                    className="mt-3 inline-flex w-full justify-center rounded-xl bg-slate-900 px-3 py-2 text-xs font-semibold text-white hover:bg-black"
                  >
                    Lieferschein
                  </button>
              ...
              <table className="w-full table-fixed border-collapse text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-slate-500">
                    ...
                    <th className="w-28 px-2 py-2">Aktion</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredRecords.map((record) => (
